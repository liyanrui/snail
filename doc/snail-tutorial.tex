...
Seven years gone in a blink and it's already 2025. Honestly I've achieved nothing remarkable. Feeling low, I revisited MetaPost and wrote some documentation. Along the way, the third version of snail took shape. Maybe this is all I'm capable of, simple tools for simple needs.
...
All that's left is to write some MetaPost code inside the \type{MPpage} environment.

To compile a \TEX\ source file \boxquote{foo.tex} into \boxquote{foo.pdf}, in your terminal or comand window, you can excute the command 
...
With help of anchor points, you can position a node by relative displacement with the snail syntax \boxquote{\type{put (node, anchor) at somewhere}}. Example \in[put-at] puts \type{bar} at \type{foo.anchors[2][8]} using \type{anchors[2][4]} of \type{bar} as its reference point.

\startexample
snail_t foo, bar;
snail(foo, "foo");
snail(bar, "bar");
put (bar, anchors[2][4]) at foo.anchors[2][8]
snailenv.set("debug", true);
showsnails foo, bar;
\stopexample
...
put (empty, anchors[0][0]) at foo.anchors[2][2];
