...
Example \in[more-strange] applies the same technique to generate more irregular node types. The \type{fulldiamond} and \type{tensecircle} macros are defined in MetaFun. The path \type{datum} is scaled and sheared fullsquare object. Note the path assigned to the module parameter \type{frame.shpae} must be enclosed in single quotation marks, so that the result is wrapped into a MetaPost's string object. 
