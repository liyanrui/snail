...
\startexample
def est = anchors[1][1] enddef; def nth = anchors[1][5] enddef;
def wst = anchors[1][9] enddef; def sth = anchors[1][13] enddef;

snail_t foo;
snail(foo, "demo");
showsnails foo;
forsuffixes it = est, nth, wst, sth:
...
snailenv = {
  text = {
     fontsize = '/BTEX\color[darkred]{BodyFontSize}/ETEX', color = '/BTEX\color[darkred]{black}/ETEX'
  },
  frame = {
  ...
  path = {
     ...
     smooth = '/BTEX\color[darkred]{true}/ETEX', offset = '/BTEX\color[darkred]{.25BodyFontSize}/ETEX', fillet = '/BTEX\color[darkred]{.3BodyFontSize}/ETEX',
...
vardef tocurve(expr p, t) =
  save n, a; numeric n; pair a[]; n := length p;
  if n < 2: p
  else:
...
\startMP
pair a[], b[];
base := origin; a1 := branch(d, 50); a2 := branch(d, 30); a3 := branch(d, 10);
...
base := a2; b1 := branch(d, 45); b2 := branch(d, 15);
branchcolor := darkblue; thickness := 1pt;
mind(b1, "first mind subbranch"); mind(b2, "second mind subbranch");
\stopMP
