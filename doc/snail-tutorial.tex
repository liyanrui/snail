...
showflows p1, p2;
annotate(p1, "$p_1$") rotatedaround(point .5 along p1, 90)
                      shifted (-7pt, -.75cm);
annotate(p2, "$p_2$"); 
\stopexample
\example[option=MP][]{Label tail transforms}{\externalfigure[figures/21.pdf]}
...
The key to the above code lies in the principle of linear interpolation. In MetaPost language, if $a_1$ and $a_2$ are both points, the syntax $t[a_1, a_2]$ is equivalent to $(1 - t) a_1 + ta_2$.

The example below creates some curve path with \type{tocurve}.
...
p1 := a1 => a2; p2 := a2 => a3;
p3 := a4 => a3; p4 := a1 => a4;
p5 := tocurve(a5 xyto a2, .75);
p6 := tocurve(a5 yxto a4, .75);
% note the usage of &&
p7 := (a5 => a6) && (a6 => a1); 

showflows p1, p2, p3, p4, p5, p6;
startsnailpathstyle
drawarrow p7 withdashes 1mm;
stopsnailpathstyle

annot(p1, "$p$"); annot(p2, "$f$");
annot(p3, "$g$"); annot(p4, "$q$");
annot(p5, "$x$"); annot(p6, "$y$");

\stopexample
\example[option=MP][]{Commutative diagram}{\externalfigure[figures/23.pdf]}
